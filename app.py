...
def get_watched_films(chat_id):
    actions = list(Action.select().where(Action.chat_id == chat_id))
    films = set()
...
def select_film(user):
    ...
    watched_films = get_watched_films(user)
    movie_to_pop = None

    while movie_to_pop is None or movie_to_pop in watched_films:
...
def send_film(id):
    ...
    if current_chat.step < 10 or len(data) < 100:
        ...
        if id not in films_for_user.keys() or len(films_for_user[id]) == 1:
            films_for_user[id], messages[id] = candidates.get_candidates(data, id)
    ...
    bot.send_message(id, '{}\n\n{}'.format(film, message), reply_markup=markups.markup_like_or_not_or_not_watched)
